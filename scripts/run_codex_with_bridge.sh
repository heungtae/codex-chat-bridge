...
ROUTER_BASE_PATH="${ROUTER_BASE_PATH:-/codex/v1}"
...
cat >"${TMP_CONFIG}" <<EOF
[routers.default]
incoming_url = "http://127.0.0.1:${BRIDGE_PORT}${ROUTER_BASE_PATH}/responses"
EOF
...
RUST_LOG="${CODEX_BRIDGE_RUST_LOG}" codex exec \
  -c "model_providers.chat-bridge={name='Chat Bridge',base_url='http://127.0.0.1:${BRIDGE_PORT}${ROUTER_BASE_PATH}',env_key='${API_KEY_ENV}',wire_api='responses'}" \
